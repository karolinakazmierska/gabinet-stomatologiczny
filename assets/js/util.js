(function ($) {
	...
	document.addEventListener("DOMContentLoaded", function () {
		...
		opinions.forEach((opinionEl) => {
			const paragraphs = opinionEl.querySelectorAll("p");
			if (!paragraphs.length) return;

			const textEl = paragraphs[0];
			const fullText = textEl.textContent.replace(/\s+/g, " ").trim();

			if (fullText.length <= MAX_CHARS) return;

			// ucinanie po ostatnim pełnym słowie
			let truncated = fullText.slice(0, MAX_CHARS);
			...
			textEl.dataset.truncatedText = truncatedText;
			textEl.dataset.expanded = "false";
			textEl.textContent = truncatedText;

			const btn = document.createElement("button");
			btn.type = "button";
			btn.className = "opinion__toggle";
